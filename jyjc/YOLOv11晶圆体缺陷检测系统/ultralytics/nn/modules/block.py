...
# 模块一：# 添加模块myCBAM
# 对应yaml文件是 yolo11-myCBAM+C3CA.yaml exp10
...
class CoordAtt(nn.Module):
    ...
    def __init__(self, channels, ratio=32):
    ...
    def forward(self, x):
        ...
        x_w = x_w.permute(0, 1, 3, 2)  # [n, mip, 1, w] -> [n, mip, w, 1]
        a_h = self.conv_h(x_h).sigmoid()  # [n, c, h, 1]
        a_w = self.conv_w(x_w).sigmoid()  # [n, c, w, 1]
...
class CABottleneck(nn.Module):
    ...
    def __init__(self, c1, c2, shortcut=True, g=1, e=0.5, ratio=32):
        ...
        self.ca = CoordAtt(c2, ratio)
        self.add = shortcut and c1 == c2

    def forward(self, x):
        x1 = self.cv2(self.cv1(x))
        x1 = self.ca(x1)  # 应用坐标注意力
...
# 模块二：对应yaml文件是 yolo11-C3ECA.yaml
# 在C3模块上添加ECA注意力机制，然后将C3K2模块替换为C3ECA模块。
